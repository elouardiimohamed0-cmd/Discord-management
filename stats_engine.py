...
PCT rating is usually already an average (1-10).
If it is cumulative, divide it.
if player.rating > 10 and player.games > 1:
player.rating_pg = round(player.rating / g, 2)
else:
player.rating_pg = round(player.rating, 2)
IMPORTANT:
Only recompute win_rate when wins/losses/draws are populated.
PCT often gives win_rate directly and leaves W/L/D as 0.
wld_total = player.wins + player.losses + player.draws
if wld_total > 0:
player.win_rate = round(player.wins / wld_total * 100, 1)
else: keep scraper win_rate
return player
@classmethod
...
player.goals * 3
player.assists * 2
player.shots_on_target * 0.5
player.key_passes * 1.5
)
def_contrib = (
player.tackles * 2
...
(player.possession_losses  1.5 + player.fouls  2 + player.cards * 5) / g,
2,
)
player.throwing_score = round(
player.error_score / max(player.rating_pg, 1.0),
2,
)
