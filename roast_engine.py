...
class RoastEngine:
    ...
    def _generate_english_roast(self, nickname: str, metrics: RoastMetrics) -> str:
        ...
        if metrics.aura_tier == AuraTier.GHOST:
            roasts.extend([
                ...
                f"{nickname} has {metrics.games} games played. That is not a player, that is a ghost haunting the club.",
                f"Even the bench has more presence than {nickname}.",
                f"{nickname} is so invisible, the opponent team does not even mark them.",
            ])
        ...
        if metrics.goals == 0 and metrics.games > 10:
            roasts.extend([
                ...
                f"{nickname} could not score in an empty net with a map and GPS.",
                f"{metrics.games} games, 0 goals. {nickname} is a defensive striker.",
                f"{nickname} shoots like they are allergic to the goal.",
            ])
        if metrics.assists == 0 and metrics.games > 10:
            roasts.extend([
                ...
                f"{nickname} passes like they are playing solo queue in a team game.",
                f"{metrics.games} games, 0 assists. {nickname} is a one-man disaster.",
            ])
        if metrics.win_rate < 30 and metrics.games > 10:
            roasts.extend([
                f"{nickname} has a {metrics.win_rate:.0f}% win rate. That is not a player, that is a curse.",
        ...
        if not roasts:
            roasts = [
                f"{nickname} is the reason we cannot have nice things.",
    ...
    def fraud_accusation(self, ea_name: str, stats: Dict) -> str:
        nickname = self.mapper.get_nickname(ea_name)
        metrics = self._build_metrics(stats)
        english = f"FRAUD ALERT: {nickname} | Fraud Score: {metrics.fraud_score:.0f}/100 | Aura: {metrics.aura_tier.value}"
    ...
    def ghost_accusation(self, ea_name: str, stats: Dict) -> str:
        ...
        english = f"GHOST DETECTED: {nickname} | Games Played: {games} | Status: Basically does not exist"
    ...
    def carry_praise(self, ea_name: str, stats: Dict) -> str:
        nickname = self.mapper.get_nickname(ea_name)
        metrics = self._build_metrics(stats)
        english = f"CARRY CONFIRMED: {nickname} | Impact: {metrics.impact} | Win Rate: {metrics.win_rate:.0f}% | Aura: {metrics.aura_tier.value}"
    ...
    def stat_of_day(self, ea_name: str, stat_name: str, value, is_roast: bool = True) -> str:
        nickname = self.mapper.get_nickname(ea_name)
        if is_roast:
            english = f"STAT OF THE DAY: {nickname} | {stat_name}: {value}"
        else:
            english = f"STAT OF THE DAY: {nickname} | {stat_name}: {value}"
