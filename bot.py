...
class DataLayer:
    ...
    def get_player_stats(self, ea_name: str) -> Optional[Dict]:
        return self.get_all_stats().get(ea_name)
    ...
    def get_recent_matches(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM matches ORDER BY match_date DESC, id DESC LIMIT ?", (limit,))
            for row in cursor.fetchall():
...
async def who_sold_cmd(interaction: Interaction):
    ...
    try:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT player_name, rating, goals, assists, possession_lost FROM matches WHERE match_id = ? ORDER BY rating ASC LIMIT 1",
                (match_id,)
            )
            row = cursor.fetchone()
            if row:
