...
threading.Thread(target=lambda: HTTPServer(("0.0.0.0", Config.PORT), HealthHandler).serve_forever(), daemon=True).start()
...
async def cmd_debug(ctx):
    """Show current bot state for troubleshooting."""
    global scraper, current_club
    lines = [
        f"**Bot User:** {bot.user}",
        f"**Scraper ready:** {'Yes' if scraper else 'No'}",
        f"**Data loaded:** {'Yes' if current_club and current_club.players else 'No'}",
        f"**Players in cache:** {len(current_club.players) if current_club and current_club.players else 0}",
        f"**PCT URL:** {Config.PCT_CLUB_URL}",
        f"**PORT:** {Config.PORT}",
        f"**Guild ID:** {Config.DISCORD_GUILD_ID}",
    ]
...
async def cmd_help(ctx):
    ...
    cmds = [
        ...
        ("`!debug`", "معلومات تقنية باش تعرف واش كاين مشكل"),
...
# ... (rest of slash commands identical to previous version, omitted for brevity)
# Include all slash commands from the previous version here
