"""aura_system.py — MINIMAL BULLETPROOF VERSION"""
...
class AuraSystem:
    ...
    def calculate_overall(self, stats: Dict[str, Any]) -> float:
        ...
        overall = (rating * 8 + (goals / games) * 15 + (assists / games) * 10 + impact * 3 + win_rate * 0.2)
        return min(99, max(60, round(overall, 1)))

    def determine_tier(self, stats: Dict[str, Any]) -> AuraTier:
        games = stats.get("games", 0)
        if games == 0 or games < 3:
    ...
    def calculate_fraud_score(self, stats: Dict[str, Any]) -> float:
        ...
        fraud = (goal_deficit * 3 + assist_deficit * 2 + impact_deficit * 5 + possession_lost * 0.5) / max(games, 1)
